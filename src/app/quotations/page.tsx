...
function QuotationsContent() {
  ...
  useEffect(() => {
    if (!isAdmin && profile?.role !== 'client' && !authLoading) {
      router.push("/");
      return;
    }
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      console.warn("Quotations Access Error:", error);
      setLoading(false);
    });
  ...
  const handleDeleteQuotation = async (id: string) => {
    if (!db || !isAdmin || !confirm("هل أنت متأكد من الحذف؟")) return;
    try {
  ...
  if (loading || authLoading) return (
    <div className="flex flex-col items-center justify-center py-20 gap-4">
      ...
      <p className="font-bold text-slate-500 text-xs">جاري تحميل المعرض...</p>
    </div>
  );
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 pb-20" dir="rtl">
      ...
      {quotations.length === 0 ? (
        <Card className="rounded-[2rem] border-none shadow-sm py-20 text-center bg-white border-2 border-dashed">
          <ImageIcon className="h-16 w-16 mx-auto mb-4 opacity-20" />
          <p className="text-xl font-black opacity-30">لا توجد عروض حالياً</p>
        </Card>
      ) : (
                  ...
                  {q.images?.[0] ? (
                    <>
                      ...
                      <div className="absolute bottom-5 right-5 flex items-center gap-3">
                        <Badge className="bg-white/20 backdrop-blur-md text-white border-none px-3 py-1 rounded-lg font-black text-[10px]">
                          {q.images.length} صور
                        </Badge>
