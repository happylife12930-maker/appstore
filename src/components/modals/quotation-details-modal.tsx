...
export function QuotationDetailsModal({ isOpen, onClose, quotation }: QuotationDetailsModalProps) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[1000px] w-[95vw] rounded-[2.5rem] border-none shadow-2xl p-0 overflow-hidden bg-white max-h-[95vh] flex flex-col" dir="rtl">
          ...
          <DialogHeader>
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex gap-2">
                ...
                {quotation.executionTimelineDays > 0 && (
                  <div className="bg-white/10 px-4 py-2 rounded-2xl border border-white/20 text-center">
        ...
        <ScrollArea className="flex-1 p-8 bg-slate-50/30">
          <div className="space-y-10">
            {/* معرض الصور التلقائي - Masonry Gallery */}
                ...
                <div className="columns-1 sm:columns-2 lg:columns-3 gap-4 space-y-4">
                  {quotation.images.map((img: string, idx: number) => (
                    <div 
                      ...
                      className="break-inside-avoid group relative rounded-3xl overflow-hidden border-4 border-white shadow-sm hover:shadow-xl transition-all cursor-zoom-in bg-slate-100"
                    >
                      <img 
                        src={img} 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
              {(quotation.suggestedRequirements?.length > 0) && (
              ...
              {(quotation.notes || quotation.estimatedCost > 0) && (
                <div className="space-y-6">
                  {quotation.notes && (
                    <div className="space-y-3">
                      <h3 className="font-black text-slate-800 flex items-center gap-2 pr-2">
                      ...
                      <div className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm min-h-[100px]">
